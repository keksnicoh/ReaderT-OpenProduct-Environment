...
import Env (Label (Label), nil, (#:))
import Test.Hspec (describe, hspec, it, shouldBe)
import TestHandler (embeddedTest, handler, labelTest)
    ...
    describe "handler" $
      do
        it "should work with underlying Identity monad" $
          let -- note: the dependency on the "bork" label does not exists due to the mocked someDependency
              env =
                "depf" #: pure @Identity time #: Label @"derp" "yo" #: nil
        ...
        it "should work with underlying IO monad" $
          do
    ...
    describe "labelTest" $
      do
        ...
        it "should work with available labels" $
          let env =
                Label @"a" "a" #: Label @"b" "b1" #: Label @"b" "b2" #: nil
              result = runIdentity $ runReaderT labelTest env
